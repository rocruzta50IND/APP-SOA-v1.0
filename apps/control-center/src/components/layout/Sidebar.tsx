  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex items-center gap-3 h-16 border-b border-white/5 relative overflow-hidden">
        <div className={cn("absolute inset-0 bg-gradient-to-r pointer-events-none", isProduction ? "from-emerald-500/5 to-transparent" : "from-orange-500/5 to-transparent")} />
        <div className={cn("p-2 rounded-lg shrink-0 border", isProduction ? "bg-emerald-500/10 border-emerald-500/20" : "bg-orange-500/10 border-orange-500/20")}>
          <Cpu className={cn("w-5 h-5", isProduction ? "text-emerald-500" : "text-orange-500")} />
        </div>
        {!isCollapsed && (
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive 
                  ? (isProduction && item.href === "/production" ? "bg-emerald-500/10 text-emerald-500 border border-emerald-500/50 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-orange-500/10 text-white border border-orange-500/20")
            ...
            >
              {isActive && (
                <motion.div 
                  layoutId="activeSide"
                  className={cn("absolute left-0 w-1 h-4 rounded-r-full", isProduction ? "bg-emerald-500" : "bg-orange-500")}
                />
              )}
              <item.icon className={cn(
                "w-5 h-5 shrink-0 transition-colors",
                isActive 
                  ? (isProduction && item.href === "/production" ? "text-emerald-500" : "text-orange-500 drop-shadow-[0_0_8px_rgba(245,158,11,0.5)]")
                  : (isProduction ? "group-hover:text-emerald-400" : "group-hover:text-orange-400")
              )} />
      ...
      {/* Bottom Actions */}
      <div className={cn("p-3 border-t border-white/5 space-y-1 bg-gradient-to-t to-transparent", isProduction ? "from-emerald-500/5" : "from-orange-500/5")}>
        <button className={cn(
